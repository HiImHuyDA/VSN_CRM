...
BEGIN
    ...
    FROM (
        SELECT 1 AS [Id], N'Tháng 1' AS [Name] UNION SELECT 2, N'Tháng 2' UNION SELECT 3, N'Tháng 3' UNION
        SELECT 4, N'Tháng 4' UNION SELECT 5, N'Tháng 5' UNION SELECT 6, N'Tháng 6' UNION
        SELECT 7, N'Tháng 7' UNION SELECT 8, N'Tháng 8' UNION SELECT 9, N'Tháng 9' UNION
        SELECT 10, N'Tháng 10' UNION SELECT 11, N'Tháng 11' UNION SELECT 12, N'Tháng 12'
    ) m
    ORDER BY [Id];
...
BEGIN
    ...
    SELECT DISTINCT p.[Project_id], p.[SubmitDate], p.[CustomerType], p.[CustomerName], 
                    p.[SubmitterName], s.[TenTrangThai] AS [Status], p.[CreatedAt], p.[UpdatedAt], p.[SubmitterMNV]
    ...
    WHERE p.[RecordType] = 1
      ...
      AND (@ProjectStatus IS NULL OR @ProjectStatus = '' OR (
          s.[TenTrangThai] IN (SELECT value FROM STRING_SPLIT(@ProjectStatus, ','))
          OR (CHARINDEX(N'Đang xử lý', @ProjectStatus) > 0 AND s.[TenTrangThai] IN (N'PRD đã duyệt', N'BOD đã duyệt'))
          OR (CHARINDEX(N'Chờ phản hồi', @ProjectStatus) > 0 AND s.[TenTrangThai] = N'Đã gửi-Chờ phản hồi')
      ))
...
PRINT 'Fix for reports stored procedures (restored exact schema/columns & mappings) migration applied successfully!';
GO
