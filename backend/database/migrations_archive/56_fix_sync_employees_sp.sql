    ...
    BEGIN TRY
        ...
        WHEN MATCHED THEN
            UPDATE SET
                ...
                [StatusId]       = 1, -- 1 = Hoạt động (thay thế IsActive = 1)
        ...
        OUTPUT $action, inserted.[MNV], inserted.[FullName], inserted.[Department] 
        INTO @MergedOutput;
        ...
        UPDATE U
        SET U.[StatusId] = 2, -- 2 = Dừng (thay thế IsActive = 0)
            U.[UpdatedAt] = GETDATE()
...
PRINT 'Fix for sync employees stored procedure (restored exact schema/columns & mappings) migration applied successfully!';
GO
