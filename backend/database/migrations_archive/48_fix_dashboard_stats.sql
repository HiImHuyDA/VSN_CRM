...
BEGIN
    ...
    LEFT JOIN [dbo].[CSR_Projects] p ON p.[RecordType] = 1 AND p.[StatusId] != 9
      AND (
          @ActorRole IN ('Admin', 'BOD', 'PRD')
          OR p.[SubmitterMNV] = @ActorMNV
      )
    ...
    INNER JOIN [dbo].[CSR_Tasks] t ON t.[Project_id] = p.[Project_id] AND t.[StatusId] = 1
    WHERE p.[RecordType] = 1 
      AND p.[StatusId] IN (5, 7) -- BOD đã duyệt, Hoàn thành
      AND (
          @ActorRole IN ('Admin', 'BOD', 'PRD')
          OR p.[SubmitterMNV] = @ActorMNV
      )
...
PRINT 'Fix for usp_Dashboard_GetStats migration applied successfully!';
GO
